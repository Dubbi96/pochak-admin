...
--
-- 1. Organization access_type (City/OPEN vs Club/CLOSED)
-- 2. Membership approval flow enhancements
-- 3. Comments table
-- 4. Follows table
-- ============================================================================
...
-- approved_at already exists from V006
-- Ensure approved_at column exists (idempotent)
ALTER TABLE content.memberships
...
CREATE TABLE IF NOT EXISTS content.comments (
    ...
    content_type    VARCHAR(20) NOT NULL,   -- LIVE / VOD / CLIP / MATCH / etc.
    user_id         BIGINT NOT NULL,        -- identity.users(id) cross-schema reference
    body            VARCHAR(2000) NOT NULL,
    parent_id       BIGINT REFERENCES content.comments(id),  -- NULL for top-level, set for replies
...
CREATE TABLE IF NOT EXISTS content.follows (
    id                  BIGSERIAL PRIMARY KEY,
    follower_user_id    BIGINT NOT NULL,        -- identity.users(id) cross-schema reference
    target_type         VARCHAR(30) NOT NULL,    -- USER / TEAM / CLUB / ORGANIZATION
