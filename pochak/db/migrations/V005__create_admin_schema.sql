...
CREATE TABLE IF NOT EXISTS admin.banners (
    ...
    sport_id        INT,                    -- content.sports(id) 참조 (cross-schema)
...
-- 신고
-- NOTE: reporter_user_id references identity.users(id) (cross-schema, FK not enforced)
-- NOTE: reported_user_id references identity.users(id) (cross-schema, FK not enforced)
CREATE TABLE IF NOT EXISTS admin.reports (
    id              SERIAL PRIMARY KEY,
    reporter_user_id BIGINT NOT NULL,       -- identity.users(id) 참조 (cross-schema)
    reported_user_id BIGINT,                -- identity.users(id) 참조 (cross-schema)
