...
export default function EliteTeamPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error("[EliteTeams] fetch error", err);
    } finally {
      setLoading(false);
    }
  }, [searchKeyword, page]);
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">개방형(PUBLIC) 단체 목록을 조회하고 관리합니다.</p>
        </div>
            ...
            {loading ? (
              ...
              items.map((item, idx) => (
                <tr key={item.id} className={`border-b border-gray-100 transition-colors hover:bg-gray-50 ${idx % 2 === 1 ? "bg-gray-50/50" : ""}`}>
                  ...
                  <td className="px-4 py-3 text-gray-600">{ORG_TYPE_LABELS[item.type] ?? item.type}</td>
                  <td className="px-4 py-3 text-center">
                    <Badge variant={item.accessType === "OPEN" ? "default" : "secondary"}>
                      {ACCESS_TYPE_LABELS[item.accessType] ?? item.accessType}
                    </Badge>
                  </td>
