...
const EXPORT_COLUMNS = [
  ...
  { header: "가입정책", accessor: "joinPolicy" },
...
export default function PublicOrgPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error("[PublicOrg] fetch error", err);
    } finally {
      setLoading(false);
    }
  }, [searchKeyword, page]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">단체(포착 시티) 관리</h1>
          <p className="mt-1 text-sm text-gray-500">개방형(OPEN) 단체 목록을 조회하고 관리합니다.</p>
        </div>
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50 text-xs font-medium uppercase tracking-wider text-gray-500">
              ...
              <th className="px-4 py-3 text-center">가입정책</th>
              ...
              items.map((item, idx) => (
                <tr key={item.id} className={`border-b border-gray-100 transition-colors hover:bg-gray-50 ${idx % 2 === 1 ? "bg-gray-50/50" : ""}`}>
                  ...
                  <td className="px-4 py-3 text-center text-gray-600 text-xs">
                    {item.joinPolicy ? (JOIN_POLICY_LABELS[item.joinPolicy as keyof typeof JOIN_POLICY_LABELS] ?? item.joinPolicy) : "-"}
                  </td>
