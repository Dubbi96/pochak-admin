...
import type { AssetVisibility, VodCreateRequest } from "@/types/content-asset";
import {
  createVodAsset,
  getVenueOptions,
...
import type { LiveAsset } from "@/types/content-asset";

export default function VodCreatePage() {
  ...
  const [sportOptions, setSportOptions] = useState<
    { id: number; name: string }[]
  >([]);
  const [liveOptions, setLiveOptions] = useState<LiveAsset[]>([]);
  ...
  useEffect(() => {
    ...
    getLiveAssets({}, 0, 100).then((res) => setLiveOptions(res.content));
  }, []);
    ...
    try {
      const data: VodCreateRequest = {
        ...
        specificUserIds: visibility === "SPECIFIC" && specificUserIds
          ...
          : undefined,
        linkedLiveId: linkedLiveId
          ? parseInt(linkedLiveId, 10)
          : undefined,
      };
      await createVodAsset(data);
      router.push("/contents/vod");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      {/* Page Header */}
      <h1 className="text-xl font-bold text-gray-900">VOD 등록</h1>
        ...
        <div className="space-y-2">
          ...
          >
            <div className="flex items-center gap-2">
              <RadioGroupItem value="B2B" id="vod-ot-b2b" />
              <Label htmlFor="vod-ot-b2b" className="font-normal">
                B2B
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="B2G" id="vod-ot-b2g" />
              <Label htmlFor="vod-ot-b2g" className="font-normal">
                B2G
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="B2C" id="vod-ot-b2c" />
              <Label htmlFor="vod-ot-b2c" className="font-normal">
                B2C
              </Label>
            </div>
          </RadioGroup>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder="https://vod.example.com/..."
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder="https://vod.example.com/pano/..."
          />
        </div>
        ...
        <div className="space-y-1.5">
          <Label>인코딩 상태</Label>
          <div>
            <Badge variant="secondary">대기 (PENDING)</Badge>
            <p className="mt-1 text-xs text-gray-400">
              등록 후 인코딩이 자동으로 시작됩니다.
            </p>
          </div>
        </div>
        ...
        {/* 연결된 LIVE */}
        <div className="space-y-1.5">
          <Label>연결된 LIVE 선택 (선택사항)</Label>
          <Select value={linkedLiveId} onValueChange={setLinkedLiveId}>
            <SelectTrigger>
              <SelectValue placeholder="연결된 LIVE 없음" />
            </SelectTrigger>
            <SelectContent>
              {liveOptions.map((l) => (
