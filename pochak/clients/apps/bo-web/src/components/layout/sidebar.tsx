...
/**
 * Menu group separators: insert a divider BEFORE these indices.
 * Groups: [0-2] base, [3-5] org/competition, [6-8] shooting/studio,
 * [9-10] content/members, [11-12] commerce/site, [13-14] support/ops, [15-17] monitoring/stats/skylife
 */
...
function SidebarMenuItem({ item }: { item: MenuItem }) {
  ...
  // Check if the user can access this menu item (or any of its children)
      ...
      <div>
        <button
          ...
          className={cn(
            ...
            isChildActive && canAccess && "bg-blue-50 text-blue-600 font-medium"
          )}
          disabled={!canAccess}
        ...
        >
          <span className={cn(
            ...
            canAccess ? (isChildActive ? "text-blue-600" : "text-gray-400") : "text-gray-300",
            "[&>svg]:shrink-0"
          )}>
            <span className={cn(
              isChildActive && canAccess ? "text-blue-600" : canAccess ? "text-gray-400" : "text-gray-300"
            )}>
              {item.icon}
            </span>
          ...
          {canAccess && (
            <span className={cn(
              ...
              <ChevronDown size={14} className={isChildActive ? "text-blue-600" : "text-gray-400"} />
            </span>
          )}
        </button>
        {expanded && canAccess && (
            ...
            {item.children!.map((child) => {
              ...
              if (!childCanAccess) {
                return (
                  <span
                    key={child.href}
                    className="flex items-center gap-2 py-2 pl-10 pr-4 text-sm text-gray-300 cursor-not-allowed"
                    title="접근 권한이 없습니다"
                  >
                    {child.icon}
                    {child.label}
              ...
              return (
                <Link
                  ...
                  href={child.href}
                  className={cn(
                    "flex items-center gap-2 py-2 pl-10 pr-4 text-sm transition-colors",
                    childActive
                ...
                >
                  <span className={childActive ? "text-blue-600" : "text-gray-400"}>
                    {child.icon}
                  </span>
                  {child.label}
                </Link>
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed left-0 top-0 z-50 flex h-full w-[220px] flex-col bg-white border-r border-gray-200 transition-transform duration-200 lg:static lg:translate-x-0",
      ...
      >
        ...
        <div className="flex h-14 items-center justify-between border-b border-gray-200 px-4">
          <Link href="/" className="flex items-center gap-2">
            <span className="text-lg font-bold tracking-tight text-gray-900">POCHAK</span>
        ...
        <div className="border-t border-gray-200 px-4 py-3">
          <p className="text-[11px] text-gray-400">POCHAK Back Office v1.0</p>
        </div>
      </aside>
    </>
  );
