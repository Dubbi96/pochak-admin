...
function TopBar({onMenuPress, translateY}: TopBarProps) {
...
const SIDEBAR_NAV_MAP: Record<string, {route: keyof RootStackParamList; params?: object} | {tab: string}> = {
  ...
  '대회소식': {route: 'CityHome'},
  '시설예약': {route: 'WatchReservation'},
  '자주가는시설': {route: 'WatchReservation'},
};
...
function LiveSection({items}: {items: LiveContentItem[]}) {
  const navigation = useNavigation<HomeNavProp>();
    ...
    ({item}: {item: LiveContentItem}) => (
      <TouchableOpacity
        ...
        <View style={styles.liveInfo}>
          <Text style={styles.liveLeague}>{item.league}</Text>
          ...
          {item.viewerCount != null && (
            <Text style={styles.liveViewers}>
              {formatViewCount(item.viewerCount)}명 시청중
            </Text>
          )}
        </View>
      </TouchableOpacity>
    ...
    <View style={styles.section}>
      <View style={styles.sectionHeader}>
        <View style={styles.liveTitleRow}>
          <View style={styles.liveRedDotLarge} />
          <Text style={styles.sectionTitle}>LIVE</Text>
        </View>
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={() => navigation.navigate('VideoList', {sectionType: 'LIVE', title: 'LIVE'})}>
          <Text style={styles.sectionMore}>{t('common.viewAll')}</Text>
        </TouchableOpacity>
      </View>
      <FlatList
        data={items}
...
function CompetitionSection({items}: {items: CompetitionItem[]}) {
  ...
  return (
    <View style={styles.section}>
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle}>{t('home.competitionInfo')}</Text>
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={() => navigation.navigate('CityHome')}>
          <Text style={styles.sectionMore}>{t('common.more')}</Text>
        </TouchableOpacity>
      </View>
      <FlatList
        data={items}
    ...
    <View style={styles.section}>
      <View style={styles.sectionHeader}>
        ...
        <TouchableOpacity activeOpacity={0.7} onPress={handleMorePress}>
          <Text style={styles.sectionMore}>{section.moreLabel}</Text>
        </TouchableOpacity>
      </View>
      <FlatList
        data={section.items}
...
export default function TVHomeScreen() {
  ...
  return (
    <View style={styles.safeArea}>
      ...
      {/* Sticky Top Bar */}
      <TopBar onMenuPress={openSidebar} translateY={topBarTranslateY} />
      ...
      <Animated.ScrollView
        ...
        {/* LIVE */}
        <LiveSection items={mockLiveContents} />

        {/* Competition Info */}
        <CompetitionSection items={mockCompetitions} />

        {/* Content Sections */}
        {mockContentSections.map(section => (
