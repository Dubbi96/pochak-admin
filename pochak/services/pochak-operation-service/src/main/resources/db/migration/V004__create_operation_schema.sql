...
CREATE TABLE IF NOT EXISTS operation.venues (
    ...
    sport_id        INT NOT NULL,           -- content.sports(id) cross-schema
...
CREATE TABLE IF NOT EXISTS operation.reservations (
    id              BIGSERIAL PRIMARY KEY,
    venue_id        INT NOT NULL REFERENCES operation.venues(id),
    match_id        BIGINT,                 -- content.matches(id) cross-schema
    reserved_by_user_id BIGINT NOT NULL,    -- identity.users(id) cross-schema
...
CREATE TABLE IF NOT EXISTS operation.studio_sessions (
    id              BIGSERIAL PRIMARY KEY,
    match_id        BIGINT NOT NULL,        -- content.matches(id) cross-schema
    venue_id        INT NOT NULL REFERENCES operation.venues(id),
